...
# group by bike type and member type and plot graph
analysis_2<- all_trips_v2 %>% group_by(rideable_type,member_casual) %>% summarise(number_of_rides=n(),average_duration=mean(ride_length)) %>% ggplot() + geom_col(mapping= aes(x=rideable_type,y=average_duration,fill=member_casual),position='dodge')